...
jsUri.prototype.parseUri = function(str) {
	var o = jsUri.options, m = o.parser[o.strictMode ? "strict" : "loose"]
			.exec(str), uri = {}, i = 14;
	while (i--)
		uri[o.key[i]] = m[i] || "";
	uri[o.q.name] = {};
...
jsUri.prototype.toString = function() {
	...
	if (is(this.path()))
		s += this.path();
	else if (is(this.host())
			&& (is(this.query().toString()) || is(this.anchor())))
		s += '/';
...
jsUri.prototype.deleteQueryParam = function(key, val) {
	...
	for ( var p in this.query().params) {
		...
		if (arguments.length == 2
				&& this.query().decode(param[0]) == this.query().decode(key)
				&& this.query().decode(param[1]) == this.query().decode(val))
			continue;
	...
	if (arguments.length == 3) {
		...
		for ( var p in this.query().params) {
			...
			if (this.query().decode(param[0]) == this.query().decode(key)
					&& decodeURIComponent(param[1]) == this.query().decode(
							oldVal)) {
				index = p;
				break;
			}
		}
		return this.deleteQueryParam(key, oldVal).addQueryParam(key, newVal,
				index);
	} else {
		var index = -1;
		for ( var p in this.query().params) {
