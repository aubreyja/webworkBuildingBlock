...
<fmt:message var="listElementName11"
	key="ListElementName.Assignment[11]" />
...
<fmt:message var="listElementLabel11"
	key="ListElementLabel.Assignment[11]" />
...
<c:if test="${empty sessionScope.failure }">
	...
	<jsp:useBean id="configUtil" class="edu.missouri.ConfigFile"
		...
		<jsp:setProperty name="configUtil" property="requestUrl"
			value="${blackboardUtil.requestUrl }" />
		<jsp:setProperty name="configUtil" property="requestUri"
			value="${blackboardUtil.requestUri }" />
	...
	<jsp:setProperty name="publishData"
		property="assignmentsInitialStartDate"
		value="${webworkData.selectedWebworkSetsOpenDate }" />
	<jsp:setProperty name="publishData"
		property="assignmentsInitialEndDate"
		value="${webworkData.selectedWebworkSetsDueDate }" />
...
<c:if test="${not empty param.save}">
	...
	<jsp:setProperty name="publishData" property="pointsPossible"
		param="pointsPossible" />
	<jsp:setProperty name="publishData" property="assignmentComments"
		param="assignmentComments" />
	<jsp:setProperty name="publishData" property="type" param="type" />
	<jsp:setProperty name="publishData" property="isAvailable"
		param="isAvailable" />
	<jsp:setProperty name="publishData" property="isTracked"
		param="isTracked" />
	<jsp:setProperty name="publishData" property="createAnnouncement"
		param="createAnnouncement" />
	<jsp:setProperty name="publishData" property="startDate"
		param="startDate" />
	<jsp:setProperty name="publishData" property="endDate" param="endDate" />
	<jsp:setProperty name="publishData" property="rePublishReset"
		value="true" />
</c:if>
<c:if test="${(empty sessionScope.failure) or (not empty param.save) }">

	<!-- Setting each of the gradebook Line item. -->
	<jsp:setProperty name="publishData" property="courseId"
...
<bbNG:genericPage ctxId="ctx">
	...
	<c:choose>
		...
		<c:when test="${not empty sessionScope.failure }">
			...
			<jsp:setProperty name="publishData" property="publishedRecord"
				value="${blackboardUtil.publishData.publishedRecord }" />
			<bbNG:form id="${formName}" method="post"
				action="${submitPage}?save=true"
				onsubmit="return validatePublishForm();">
					...
					<bbNG:step title="${stepTitle}">
						<bbNG:dataElement isRequired="true" label="${dataElementLabel}"
							...
							<bbNG:inventoryList listId="${inventoryListId}"
								...
								<bbNG:listElement name="${listElementName1}" isRowHeader="true"
									label="${listElementLabel1}">
									<bbNG:textElement name="name"
										isDisabled="${eachAssignmentSuccess}"
										value="${publishData.name[index]}" isRequired="true"
								...
								<bbNG:listElement name="${listElementName5}"
									...
									<bbNG:dataElement isRequired="true" isVertical="true">
										<bbNG:checkboxElement name="isAvailable" value="true"
											optionLabel="${chkYes}"
											isSelected="${publishData.isAvailable[index]}"
											isDisabled="${(not publishData.isTracked[index]) or eachAssignmentSuccess}"
											onchange="selectCheckbox();" isVertical="true" />
										<bbNG:checkboxElement name="isAvailable" value="false"
											optionLabel="${chkNo}"
											isSelected="${not publishData.isAvailable[index]}"
											isDisabled="${(publishData.isTracked[index]) or eachAssignmentSuccess}"
											onchange="selectCheckbox();" isVertical="true" />
									</bbNG:dataElement>
								</bbNG:listElement>
								<bbNG:listElement name="${listElementName11}"
									label="${listElementLabel11}">
									<bbNG:dataElement isRequired="true" isVertical="true">
										<bbNG:checkboxElement name="isTracked" value="true"
											optionLabel="${chkYes}"
											isSelected="${publishData.isTracked[index]}"
											isDisabled="${(not publishData.isTracked[index])or eachAssignmentSuccess}"
											onchange="selectCheckbox();" isVertical="true" />
										<bbNG:checkboxElement name="isTracked" value="false"
											optionLabel="${chkNo}"
											isSelected="${not publishData.isTracked[index]}"
											isDisabled="${(publishData.isTracked[index]) or eachAssignmentSuccess}"
											onchange="selectCheckbox();" isVertical="true" />
									</bbNG:dataElement>
								</bbNG:listElement>
								<bbNG:listElement name="${listElementName6}"
									...
									<bbNG:checkboxElement name="createAnnouncement" value="true"
										optionLabel="${chkYes}"
										isSelected="${publishData.createAnnouncement[index]}"
										isDisabled="${(not publishData.createAnnouncement[index]) or eachAssignmentSuccess}"
										onchange="selectCheckbox();" isVertical="true" />
									<bbNG:checkboxElement name="createAnnouncement" value="false"
										optionLabel="${chkNo}"
										isSelected="${not publishData.createAnnouncement[index]}"
										isDisabled="${(publishData.createAnnouncement[index]) or eachAssignmentSuccess}"
										onchange="selectCheckbox();" isVertical="true" />
								</bbNG:listElement>
								<bbNG:listElement name="${listElementName7}"
									...
									<bbNG:textElement name="pointsPossible"
										isDisabled="${eachAssignmentSuccess}"
										value="${publishData.pointsPossible[index]}" isRequired="true"
										minLength="1" maxLength="3" size="4" type="float"
										helpText="${listElementDescription7}" />
								</bbNG:listElement>
								<bbNG:listElement name="${listElementName8}"
									label="${listElementLabel8}">
									<bbNG:textElement name="startDate"
										isDisabled="${eachAssignmentSuccess}"
										helpText="${listElementDescription8}" isRequired="true"
										value="${publishData.startDate[index]}" maxLength="11"
										size="12" onclick="errorSelection();" />
								</bbNG:listElement>
								<bbNG:listElement name="${listElementName9}"
									label="${listElementLabel9}">
									<bbNG:textElement name="endDate"
										isDisabled="${eachAssignmentSuccess}"
										helpText="${listElementDescription9}" isRequired="true"
										value="${publishData.endDate[index]}" maxLength="11" size="12"
