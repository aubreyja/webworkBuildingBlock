...
<jsp:useBean id="blackboardModifyUtil" scope="session"
...
<c:choose>
	<c:when test="${not empty param.save}">
		...
		<jsp:useBean id="modifyData" class="edu.missouri.PublishData"
			scope="page" />
		...
		<jsp:setProperty name="modifyData" property="startTime" param="endDate_time" />		
		<jsp:setProperty name="modifyData" property="title" param="titletext" />
		<jsp:setProperty name="modifyData" property="modifyRecord"
			value="true" />

		<!-- Setting the data inside blackboard for writing to blackboard DB. -->
		<jsp:setProperty name="blackboardModifyUtil" property="publishData"
			value="${modifyData}" />
		<jsp:setProperty name="blackboardModifyUtil"
		...
		<jsp:setProperty name="configUtil" property="successLocation"
			value="${blackboardModifyUtil.courseContentsUrl }" />
	...
	<c:otherwise>
		<bbNG:genericPage ctxId="ctx" title="Modify Webwork content">
			...
			<jsp:setProperty property="blackboardUser"
				name="blackboardModifyUtil" value="${ ctx.user }" />
			<jsp:setProperty property="courseNumber" name="blackboardModifyUtil"
				param="course_id" />
			<jsp:setProperty property="contentNumber" name="blackboardModifyUtil"
				param="content_id" />
			<jsp:setProperty property="contentNumber" name="blackboardModifyUtil"
				param="content_id" />
			<jsp:setProperty name="blackboardModifyUtil" property="requestUrl"
				value="${pageContext.request.requestURL }" />
			<jsp:setProperty name="blackboardModifyUtil" property="requestUri"
			...
			<c:set var="title"
				value="${blackboardModifyUtil.contentModify.title}" />
			<c:set var="validPage" scope="session"
				value="${blackboardModifyUtil.courseContentsUrl }" />
			...
			<bbNG:form id="modifyForm" action="modify.jsp">
				...
				<bbNG:dataCollection hasRequiredFields="true">
					<bbNG:step title="Mandatory info">
						<bbNG:dataElement label="Enter Assignment Title:" labelFor="title"
							isVertical="true">
							<bbNG:textbox name="title" text="${title}" rows="3"
								helpText="Edit the title info. Do not change any parameter info, if not sure." />
						</bbNG:dataElement>
						<bbNG:dataElement label="Start Date:" isVertical="true"
							labelFor="startDate">
							<bbNG:datePicker baseFieldName="startDate"
								dateTimeValue="${blackboardModifyUtil.contentModify.startDate}"
								isRequired="true" showTime="true" showDate="true" />
						</bbNG:dataElement>
						<bbNG:dataElement label="End Date:" isVertical="true"
							labelFor="endDate">
							<bbNG:datePicker baseFieldName="endDate"
								dateTimeValue="${blackboardModifyUtil.contentModify.endDate}"
								isRequired="true" showTime="true" showDate="true" />
						</bbNG:dataElement>
					...
					<bbNG:step title="Extra info">
						<bbNG:dataElement label="Availability:" isVertical="true">
							<bbNG:radioElement name="isAvailable" value="true"
								isSelected="${ (blackboardModifyUtil.contentModify.isAvailable eq true)? true : false}">
								True
							</bbNG:radioElement>
							<bbNG:radioElement name="isAvailable" value="false"
								isSelected="${ (blackboardModifyUtil.contentModify.isAvailable eq false)? true : false}">
								False
							</bbNG:radioElement>
						</bbNG:dataElement>
						<bbNG:dataElement label="Tracking details:" isVertical="true">
							<bbNG:radioElement name="isTracked" value="true"
								isSelected="${ (blackboardModifyUtil.contentModify.isTracked eq true)? true : false}">
								True
							</bbNG:radioElement>
							<bbNG:radioElement name="isTracked" value="false"
								isSelected="${ (blackboardModifyUtil.contentModify.isTracked eq false)? true : false}">
								False
							</bbNG:radioElement>
						</bbNG:dataElement>
						<bbNG:dataElement label="Comments:" isVertical="true"
							labelFor="body">
							<bbNG:textbox name="body" rows="1" isSpellcheckOnly="true"
								ftext="${blackboardModifyUtil.contentModify.body}"
								helpText="Enter the comments." />
						</bbNG:dataElement>
					</bbNG:step>
