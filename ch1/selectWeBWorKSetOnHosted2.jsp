	...
	<%
	    ...
	    try // NOTE: we may encounter a DB error while trying to retrieve the available assignments for the selected course. We should handle somehow this error.
		{
			...
			if ( request.getParameter( "selectedCourses" ) == null )
				response.sendRedirect( "selectWeBWorKCourseOnHosted2.jsp?content_id=" + request.getParameter( "content_id" ) + "&course_id=" + request.getParameter("course_id") + "&error=true" );
			...
			// get the name of the course we are assigning homework to; This is the WeBWorK course
			...
			// if the user is not found in the list of users from the WeBWorK database, raise the invalidUserFlag. The user does not exist in the course list.
			if ( index == listOfUsersInTheWeBWorKCourse.length )
				invalidUserFlag = true;
			////////////// The user existencce in the course list has been checked and proper flags have been raised //////////////
	...
	%>
			<bbUI:docTemplate title="New WeBWorK Assignment">
				...
				<%	
					...
					if ( invalidUserFlag ) 
					{
						String urlToAuthorizationPage = "enrollInCourseOnHosted2.jsp?content_id=" + request.getParameter( "content_id" ) + "&course_id=" + request.getParameter("course_id") + "&selectedCourses=" + request.getParameter( "selectedCourses" );
				...
				%>		
					<form name="theform" action="assignWeBWorKSetOnHosted2.jsp" method="post">
